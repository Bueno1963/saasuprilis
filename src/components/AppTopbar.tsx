  ...
  return (
    <header className="h-14 bg-sidebar text-sidebar-foreground border-b border-sidebar-border flex items-center px-5 shrink-0 relative z-50">
      ...
      <Link to="/" className="flex items-center gap-2.5 mr-8 shrink-0">
        ...
        <span className="text-sm font-semibold uppercase tracking-widest text-sidebar-foreground truncate max-w-[140px]">
          {branding?.name || "LIS"}
        </span>
      </Link>
        ...
        {profile && (
          <div className="flex items-center gap-2.5">
            ...
            <div className="hidden lg:block">
              <p className="text-xs font-medium text-sidebar-accent-foreground leading-tight truncate max-w-[120px]">
                {profile.full_name || "Usuário"}
              </p>
              <p className="text-[10px] text-sidebar-muted leading-tight">{profile.role_display}</p>
            </div>
          </div>
        )}
        <button
          onClick={signOut}
          className="text-sidebar-muted hover:text-sidebar-accent-foreground transition-colors p-2 rounded-md hover:bg-sidebar-accent"
          title="Sair"
        >
