...
import { Phone, ChevronDown, Syringe } from "lucide-react";
...
const vacinas = [
  "Vacina Pneumocócica 20", "Pneumocócica ou Pneumo-15", "Herpes Zóster",
  "Dengue", "BCG", "Hepatite B", "Hepatite A", "Rotavírus",
  "Pentavalente", "Hexavalente Acelular", "Pentavalente Acelular",
  "Pneumocócica ou Pneumo-13", "Pneumocócica ou Pneumo-23",
  "Meningocócica ACWY", "Meningocócica B", "Influenza Tetravalente (gripe)",
  "Febre Amarela", "Tetra Viral", "Tríplice Viral", "HPV Quadrivalente",
  "HPV Nonavalente", "Varicela", "Tríplice Bacteriana Acelular",
...
const VacinasPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background font-sans">
      <LandingNavbar />
      <LandingBreadcrumb items={[{ label: "Institucional" }, { label: "Vacinas" }]} />
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-2">
            {vacinas.map((vacina) => (
              <div key={vacina} className="group py-4 border-b-2 border-[hsl(205,78%,45%)] cursor-pointer hover:border-[hsl(205,78%,35%)] transition-colors">
                <div className="flex items-center gap-3">
                  <Syringe className="w-4 h-4 text-[hsl(205,78%,45%)] shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
                  <span className="text-sm font-medium text-foreground group-hover:text-[hsl(205,78%,45%)] transition-colors">{vacina}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-16 md:py-20 bg-[hsl(205,78%,28%)]">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">Agende sua vacinação</h2>
